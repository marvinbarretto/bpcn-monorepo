    ...
    if (slug) {
      ...
      return this.pageService.getPageBySlug(actualSlug).pipe(
        map((page: Page | null) => {
          ...
          console.log(`PageResolver: Page found with slug ${actualSlug} and full path ${fullPath}`);

          // If the full path already matches the current state URL, do nothing
          if (fullPath === state.url) {
            console.log('PageResolver: Full path already matches current URL, no redirect needed.');

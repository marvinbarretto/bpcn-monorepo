...
export class SearchService extends StrapiService {
   ...
   * @param query The search query string
   */
  sitewideSearch(query: string, pageSize: number = 5): Observable<{ events: Partial<IEvent>[]; pages: Page[] }> {
    const endpoint = `custom-search?query=${encodeURIComponent(query)}&pageSize=${pageSize}`;

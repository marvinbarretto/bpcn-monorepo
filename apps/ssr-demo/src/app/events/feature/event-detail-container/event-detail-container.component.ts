...
@Component({
    ...
    template: `
      ...
      <section class="event-fallback">
        ...
        <div *ngIf="!eventStore.loading$() && !eventStore.error$()" class="event-not-found">
    ...
    ngOnInit(): void {
      ...
      if (slug) {
        this.eventStore.load(slug).then(() => {
          ...
          this.meta.updateTag({ name: 'description', content: event.seo?.metaDescription || '' });
          this.meta.updateTag({ name: 'keywords', content: event.seo?.keywords || '' });

          this.meta.updateTag({ property: 'og:title', content: event.title });
          this.meta.updateTag({ property: 'og:description', content: event.seo?.metaDescription || '' });
          this.meta.updateTag({ property: 'og:image', content: resolveAssetUrl(event.hero?.formats?.large?.url || event.hero?.url) || '' });
